...
export function validateInvitationData(data: Record<string, unknown>) {
  ...
  return {
    ...
    sanitizedData: {
      ...
      text_font_family: validFonts.includes(data.text_font_family as string) ? (data.text_font_family as any) : 'inter',
      // Pass through overlay settings with defaults
      design_id: (data.design_id as string)?.trim() || undefined,
      text_overlay_style: (data.text_overlay_style as any) || 'light',
      text_position: (data.text_position as any) || 'center',
      text_size: (data.text_size as any) || 'large',
      text_shadow: data.text_shadow ?? true,
      text_background: data.text_background ?? false,
